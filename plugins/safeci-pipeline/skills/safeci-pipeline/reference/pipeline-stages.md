...
The SafeCI pipeline runs five security scanners sequentially. It is defined in
...
| 5 | ZAP        | DAST baseline| `zap_baseline.yml`     | `GITHUB_TOKEN` (auto; no issue writing)           |
...
- **Stage 5 (ZAP) is DAST** — it scans a **running web target** (`target_url`,
  default `https://ginandjuice.shop`), not the repo files. Overriding
  `target_url` points it at a different live application.
